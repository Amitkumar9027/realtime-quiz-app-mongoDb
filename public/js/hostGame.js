...
let round = {rounds : null , current : 1}
...
socket.on('gameQuestions', function (data) {
    ...
    console.log("ended game Quesions")
});
...
socket.on('updatePlayersAnswered', function (data) {
    document.querySelector('.header-info span:nth-child(2)').innerHTML = "Answered " + data;
});
    ...
    if (currentQuestionIndex == 1) {
        if(params.quiz_type == 0 || params.quiz_type == 1){
            ...
            if(round.current == 1){
                ...
                headerInfo[2].textContent = params.quiz_type == 2 ? params.no_mcq : params.no_buzzer
            }else if(round.current == 2){
                headerInfo[2].textContent = '2 / 2';
                headerInfo[2].textContent = params.quiz_type == 3 ? params.no_mcq : params.no_buzzer
            }
        }
    }
}
