...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, `${gameId}_${file.originalname}`);
  }
});
...
// File upload handler
app.post("/upload", upload.fields([{ name: 'image-picker' }, { name: 'audio-picker' }]), (req, res) => {
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    try {
      ...
      for (var i = 0; i < games.games.length; i++) {
        ...
        if (params.pin == games.games[i].pin) {
          ...
          //get name of players
          var playersInGame = players.getPlayers(hostId).map(player => player.name);
  ...
  socket.on("player-join-game", (data) => {
    ...
    try {
      ...
      if (player) {
        ...
        console.warn(`Player not found with id: ${data.id}`);
        socket.emit("noGameFound"); // No player found
      }
    } catch (error) {
        ...
        .toArray(function (err, res) {
          ...
          if (que_index <= res[0].mcq_arr.length + res[0].buzzer_arr.length) {
            ...
            if (questionData) {
              ...
              io.to(game.pin).emit("nextQuestionPlayer",{"questionData" : questionData_});
    ...
    if (game.gameData.questionLive) {
      ...
      if (num >= 1 && num <= 4) {
        ...
        if (game.gameData.playersAnswered === playerNum.length) {
          ...
          io.to(game.pin).emit("updatePlayersAnswered", game.gameData.playersAnswered);
        }
      }
    } else {
  ...
  socket.on("playerBuzzered",()=>{
    ...
    if(!game.gameData.buzzerPressed){
      ...
      io.to(game.pin).emit("updateBuzzerList" , {
        "hostId" : player.hostId,
        "PlayerId" : player.playerId,
        "name" : player.name
      })
